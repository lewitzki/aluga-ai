...
type PageProps = {
    tools: LaravelPaginator<MyToolRow>;
};

export default function MyToolsIndex({ tools: toolPage }: PageProps) {
    ...
    function confirmDelete(tool: MyToolRow) {
        if (
            !window.confirm(
                `Remover "${tool.name}"? Esta ação pode ser desfeita apenas por suporte técnico.`,
            )
        ) {
            return;
        }

        router.delete(myTools.destroy.url(tool.id));
    }

    return (
        <>
            <Head title="Minhas Ferramentas" />

            <div className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <Heading
                        title="Minhas Ferramentas"
                        description="Cadastre e gerencie as ferramentas que você disponibiliza para aluguel."
                    />
                    <Button asChild size="sm">
                        <Link href={myTools.create.url()} prefetch>
                ...
                {toolPage.data.length === 0 ? (
                    <p className="text-center text-sm text-muted-foreground">
                        Você ainda não cadastrou nenhuma ferramenta.
                    </p>
                ) : (
